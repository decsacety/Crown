...
namespace crown
{
    ...
    // A temporary memory allocator that primarily allocates from a 
    // local stack buffer of size BUFFER_SZIE. If that memory is exhausted
    // it will use the backing allocator (typically a scratch allocator)
    //
    ...
    struct TempAllocator : public Allocator
    {
        ...
        unsigned int _chunk_size;  //Chunks to allocate from backing allocator
        ...
        // Deallocation is a NOP for the TempAllocator. The memory is autromatically
        // deallocated when the TempAllocator is destroyed.
        virtual void deallocate(void *){}

        // Retuens SIZE_NOT_TRACKED
    ...
    // If possible, use one of these predefined size for the TempAllocator to avoid
