...
namespace crown
{
	...
	//Functions to mainpulate Array
	namespace array
	{

		//Returns whether the array ~a~ is empty
		template <typename T> bool empty(const Array<T>& a);

		// Return the number of items in the array ~a~
		template <typename T> u32 size(const Array<T>& a);

		// Return the maximum number of items the array 'a' can hold 
		template <typename T> u32 capacity(const Array<T>& a);

		// Resizes the array 'a' to the given 'size'
		// 
		//Old items will be copied to the newly created arry
		//If the new capacity is smaller than the previous one, the 
		// array will be truncated
		template <typename T> void resize(const Array<T>& a, u32 size);

		// Reserves space in the array 'a' for at least ~capacity~ items
		template <typename T> void reserve(Array<T>& a, u32 capacity);

		// Sets the capacity of array 'a'
		template <typename T>void set_capacity(Array<T>& a, u32 capacity);

		// Grows the array 'a' to contain at least 'min_capacity' items
		template <typename T> void grow(Array<T>& a, u32 min_capacity);

		// Condenses the array 'a' so that its capacity matches the actual number
		// of items in the array
		template <typename T> void shrink_to_fit(Array<T>& a);

		// Appends an items to the array 'a' and returns its index/
		template <typename T> u32 push_back(Array<T>& a, const T& item);

		//Removes the last items from the array 'a'
	...
	{
		CE_ASSERT(index < _size, "Index out of bounds");//要实现越界读取就修改这里
		return _data[index];
	}
